...
THIS IS AN OLD GUIDE. The premise of the goal stays the same but the actualy execution is different now.
...
### Phase structure
- Phase 1: 96 trials (training charts: delta0_S8, delta05_S8, delta0_S16, delta05_S16)
- Catch trial 1 (press 'g' to continue)
- Phase 2: 96 trials (testing charts: delta025_S8, delta025_S16, delta075_S16, delta05_S32)
- Lottery screen (one random trial selected for payment)


### Trial counterbalancing
Each phase block uses 4 location counterbalance combos × 2 repeats. The 4 combos are `[up,down]`, `[down,up]`, `[left,right]`, `[right,left]`, so each color (red/blue) appears at every position exactly twice per phase. Formula: nCharts × 3 symbols × 4 combos × 2 repeats.
...
### How sync works - This has changed to run locally
...
Before the sync fixes, after ~90–96 trials the two computers drifted by about one trial. The root cause was each client calling `phaseStartTime = Date.now()` independently at the instructions→baseline transition. The fix: both clients derive `phaseStartTime` from the same Firebase server timestamp (`trialSyncTime`).

### Phase boundary indices (trial sequence)
- phase2Start = 97 (index 96 = catch trial 1, index 97 = first Phase 2 trial)
- phase3Start = 194 (index 193 = catch trial 2, index 194 = first Phase 3 trial)
- These are computed dynamically in `loadTrialsFromData` by scanning for `isCatchTrial` flags — do not hardcode.
...
## Config (experiment.js top) - These are now changed to be a variabe length
```
...
## GOAL
Your goal and this is the top priority is make sure the syncing remains working. It currently works now. Our main goal now is to make sure they stay synced for the entire experiment and show the same thing to both players.
